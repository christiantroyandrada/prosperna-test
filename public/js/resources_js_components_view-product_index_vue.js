...
/* harmony default export */ const __WEBPACK_DEFAULT_EXPORT__ = ({
  ...
  methods: {
    read: function read() {
      ...
      axios.get('/api/items/' + this.$route.params.id).then(function (data) {
        ...
        console.log(_this.product); //load the product information and used in this component
      })["catch"](function (err) {
        console.log(err);
      });
    },
    setLoaded: function setLoaded() {
      ...
      //function of PayPal API
      this.loaded = true;
      window.paypal.Buttons({
        createOrder: function createOrder(data, actions) {
          return actions.order.create({
            purchase_units: [{
              description: _this2.product.name,
              amount: {
                currency_code: "USD",
                value: _this2.product.price
              }
            }]
          });
        },
            ...
            return _babel_runtime_regenerator__WEBPACK_IMPORTED_MODULE_0___default().wrap(function _callee$(_context) {
              while (1) {
                switch (_context.prev = _context.next) {
                  ...
                  case 2:
                    order = _context.sent;
                    _this2.form.item_id = _this2.$route.params.id; //portion of code where the order object is being transferred to the form for saving to local DB

                    _this2.form.paypal_order_id = order.id;
        ...
        onError: function onError(err) {
          //function of PayPal API when there is an error
          console.log(err);
  ...
  mounted: function mounted() {
    var script = document.createElement("script"); //mount PayPal script

    script.src = "https://www.paypal.com/sdk/js?client-id=ASqr1LAJQXq7DI7T3JMQvyYaK3EM4mMSM6PZZd7-dQw0VYlOF2Bbg9UJCNA7tnU-1tSlBtLpCQUm0LNN";
...
var render = function () {
  ...
  return _c("v-container", [
    ...
    _vm.paidFor
      ? _c("div", [
          _c("h1", [
            _vm._v("Noice, you bought a beautiful " + _vm._s(_vm.product.name)),
          ]),
        ])
      : _vm._e(),
  ])
